...
\end{flushleft}
As a comparison for the structure and dynamics of the quasi-liquid
...
wide variety of order parameters. Here, we have chosen two structural
...
computed for that bin. The results for the basal and prismatic systems
are presented in Figures \ref{fig:basal_rhoq} and
\ref{fig:prism_rhoq}.
...
for the basal systems shows a formation of a QLL at $\sim\pm$ 21. The
peaks around these displacements shows wetting of the underlying
crystal. That is, the density does not go to zero between the crystal
planes. Interior to this, the characteristic bilayer of ice is
observed in the twin peaks at each plane of ice. Note the outter peaks
have lost this bilayer distribution, further indicating its
liquid-like character. Comparing the density profiles for the four
temperatures investigated, we observe a larger amount of wetting of
the underlying crystal for warmer temperatures. Similarly, the height
of the QLL peak decreases with warmer temperature, indicating a more
...
to be $\sim$~1.5 - 2.0 $\mathrm{g}~\mathrm{cm}^{-3}$, we observe
...
order parameter profiles through the interface. XXX
...
The pairwise distribution function, $\mathrm{g}(\mathrm{r})$, provides
...
querry the orientational ordering, the density of local tetrahedral
order parameters observed during the simulation was computed (see
\ref{fig:gofrQ}).


\subsection{Dynamics Measures of Supercooled Water}
...
bulk liquid boxes. In Chapter \ref{chap:Dyn}, we computed a spatially
...
\caption{\label{fig:bulkD} Mean squared displacements for supercooled
  liquid water. }
\end{figure*}                
...
% Previous estimates of the viscosity of the quasi-liquid layer (QLL) on
