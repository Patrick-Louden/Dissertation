...
arises when considering a macroscopic number of particles
($10^{23}$), as the initial calculation of the forces isn't even
feasible. Instead, we turn to statistical mechanics as a connection
...
of the $6N$ position and momenta coordinates of the $N$ particles. The
second term in the integrand, $ \rho(\mathbf{r}^N,\mathbf{p}^N)$, is
the \textit{probability density} of the ensemble. It is a function
which describes the relative probability of observing a specific
configuration and distribution of momenta for the $N$ particles, given
the energy of the configuration. If one is able to compute $\langle A
\rangle$ by integrating over all possible configurations of the
system, the resulting ensemble average of $A$ will be equal to the
time average value of $A$ by the \textit{ergodic hypothesis}. 
...
suitable parameters for these functional forms; this gives rise to a
brief overview of the body of literature surrounding water models.

Due to computational expense, it is common to truncate the many-body
expansion of the potential energy to only the two-body interaction
term, and consider only those two-body terms which are pairwise
additive. That is, the potential
\begin{equation}\label{eq:potPair}
\mathscr{U}(\mathbf{r}^N) = \sum_{i<j}^N U(r_{i,j})
\end{equation}
with $r_{i,j}$ being the scalar distance between particles $i$ and
$j$, $r_{i,j} = | \mathbf{r}_i - \mathbf{r}_j|$. While this may seem
like an extreme restriction on the functional form of the forcefield
for our simulations, as we will see models of this form still perform
quite well. Generally, the terms incorporated into Equation
\ref{eq:potPair} fall into one of two categories, inter- and
intra-molecular interactions. 

In 1924, J. E. Lennard-Jones proposed a pairwise additive
intermolecular potential for a system of soft-spheres.
\begin{equation}\label{eq:LJ}
U(r_{i,j}) = k\epsilon\Bigg[ \Big( \frac{\sigma}{r_{i,j}}\Big)^n - \Big(\frac{\sigma}{r_{i,j}}\Big)^m\Bigg]
\end{equation}
where
...
U(r_{i,j}) =
4\epsilon\Bigg[\Big(\frac{\sigma}{r_{i,j}}\Big)^{12}-\Big(\frac{\sigma}{r_{i,j}}\Big)^{6}\Bigg]
\end{equation} 
The remaining two parameters to the model are $\sigma$, the distance
at which $U(r_{i,j})$ goes to zero, and $\epsilon$, the potential
energy minimum which is located at $\sqrt[6]{2}\sigma$. While the
...
The second intermolecular interaction we must consider is the
treatment of charged particles. 
