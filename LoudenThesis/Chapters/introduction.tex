...
of reasoning based on probabilities. Within this work, he describes
the notion of casual or scientific determinism, that is, that all
events are dictated by previously existing causes.
...
relinquishing unobtainable predictablility of nature for
representative models, stepping slowly towards understanding the
universe. We have replaced the `powerful intellect` with computers,
and actively develop algorithms to recursively solve these complex
models. Through computer simulations we are becoming more proficient
at understanding and predicting the world around us, from the motion
of planets, to the properties of molecules.
...
classical case. Several excellent books have been published which
cover molecular dynamics, from the classic text of Allan and Tildesey,
to the modern broad work of Leach.\cite{AllanXX,LeachYY}
...
terms leads to a more accurate water model.\cite{PaesaniXX} These
details will be discussed in more details in Section
\ref{sec:WaterModels}, but for now we will restrict ourselves to a
many-body expansion up to the two-body term. We further restrict
ourelves to a potential where no dissapative forces act between
...
forces acting upon those particles will change every time they move,
or one of the particles they are interacting with moves. Due to this,
the motion of all particles are coupled, giving rise to a many-body
problem with no analytic solution. Therefore, finite difference
methods of integration must be used in order to propogate the
particles through time.

While there exist a large number of algorithms for propogating
particles through time under a potential as described above, they are
not all equal. The general idea for each method is to take small steps
through time ($\delta t$) by knowing the position and momenta for
each of the $N$ particles at a previous time. Knowing these
quantities, the forces are calculated from the position, and the
positions and momenta are updated accordingly. This results in a new
$\mathbf{r}^N$ configuration, and the method iterates again. 
...
and velocities can be integrated through time using Newton's Second
Law, and for each configuration of the system
$A(\mathbf{r}^N(t),\mathbf{p}^N(t))$ could be computed. The issue
arises when considering a macroscopic number of particles ($10^{23}$),
as calculation of even the initial time step is infeasible. Instead,
we turn to statistical mechanics as a connection between a microscopic
system ($10^2 - 10^6$ particles) and the macroscopic experimental
observation. Here, instead of considering one large system, we evolve
a large number of replications of a smaller systems
simultaneously. The average value of the property from these replicas
is denoted as an \textit{ensemble average} with angle bars, $\langle$~
$\rangle$.
...
of the $6N$ position and momentum coordinates of the $N$
...
Does AandT have something on this in relation to computer time?

\subsubsection{Calculation of Thermodynamic Properties}
As the particles are evolving through time, there are several possible
thermodynamic properties which we may wish to compute. Among the
properties most commonly desired to be known are the temperature and
pressure of the system of particles.  The instantaneous temperature,
$T$, of a collection of $N$ molecules is described by their kinetic
energy $K$ through the equipartition theorem.
...
present functional forms of a forcefield commonly used for simulations
of water, followed by a discussion of suitable parameters in a brief
...
which are pairwise additive. That is, the potential
...
$r_{ij} = | \mathbf{r}_i - \mathbf{r}_j|$. While this may seem like
an extreme restriction on the total potential, however, we will see
models of this form still perform quite well. Generally, the terms
incorporated into Equation \ref{eq:potPair} fall into one of two
...
term, and all the potential depends upon is the relative distance
between particles. 
...
potential is a function of the angle formed between three
...
U(r_{ij}) = k\epsilon_{ij}\Bigg[ \Big( \frac{\sigma_{ij}}{r_{ij}}\Big)^n - \Big(\frac{\sigma_{ij}}{r_{ij}}\Big)^m\Bigg]
\end{equation}
where
...
at which $U(r_{ij})$ goes to zero, and $\epsilon_{ij}$, the potential
energy minimum which is located at $r_{ij} = \sqrt[6]{2}\sigma$. While the
...
molecules from collapsing onto one another. Due to the long-ranged
$r^{-1}$ decay of electrostatic interactions, calculating
charge-charge interactions is often the most computationally demanding
part of a molecular dynamics simulation. While there are many
implementations of how to compute these interactions, Coulomb's law is
at the heart of the methods.
\begin{equation}\label{eq:coulomb}
U_{charge}(r_{ij}) = \sum_{ij}\frac{q_i q_j e^2}{4 \pi \epsilon_0
  r_{ij}}
\end{equation}
...
Number of bonded terms goes as $N$, but number non-bonded interactions
goes as $N^2$. For LJ interactions, the potential at $2.5\sigma$ is just $1 \%$ of its
value at $\sigma$. This is due to rapidly falling off potential of
$r^{-6}$, and is commonly dealt with by implementing a cutoff radius
for non-bonded interactions, while applying a minimum image
convention. Any interactions with molecules beyond the cutoff are set
to zero. Make cutoff small enough that particle doesn't see its own
...
%%%%%%%%%%%%%%%%%%%%%%%%
\section{H$_2$O: A Simple Molecule with Mighty Implications}
