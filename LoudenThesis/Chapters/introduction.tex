...
via the Einstein's relation\cite{33}
\begin{equation}
...
$D^{*}_\mathrm{i}$ according to\cite{16}
...
method of Pfalzgraff \text{et al.}\cite{16}, where they included every
...
liquid water with the NE6 potential,\cite{18} Gladich computed
...
the TIP4P/Ice water model,\cite{Conde} and it was found that the NE6
...
mechanism proposed by Bishop \textit{et al.}\cite{15} and by Bolton
and Pettersson\cite{14}, which suggested the outtermost molecules of
...
surfaces.\cite{34} Gladich's computed values for surface diffusivity
...
experimental work reported by Price \textit{et al.}\cite{35},
...
supercooled bulk liquid\cite{37,38} have also reproduced surface
