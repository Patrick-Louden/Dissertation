...
\mathscr{U}(\mathbf{r}^N) = \sum_i^N U(\mathbf{r}_i) + \sum_{i<j}^N U(r_{i,j})
\end{equation}
where $\mathbf{r}_i$ is the absolute position of particle $i$, and
$r_{i,j}$ is the scalar distance between particles $i$ and $j$,
$r_{i,j} = | \mathbf{r}_i - \mathbf{r}_j|$. While this may seem like
